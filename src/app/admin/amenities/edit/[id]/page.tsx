...
export default function AdminEditAmenityPage() {
  ...
  useEffect(() => {
    const fetchAmenity = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/admin/amenities/${id}`, {
          headers: { "Authorization": `Bearer ${token}` }
        });
        
        if (!res.ok) {
          throw new Error("Failed to load amenity data");
        }
        ...
        setFormData({
          ...
          slug: data.slug || "",
          is_active: data.isActive !== undefined ? (data.isActive ? 1 : 0) : (data.is_active || 1)
        });
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(payload?.message || "Failed to update amenity");
      }
  ...
  if (loading) {
    return <ModernLoader text="Loading Amenity Data..." />;
  }
  ...
  return (
    <div className="!p-6 !w-full !space-y-6">
      <div className="!flex !items-center !gap-4">
        ...
        <div>
          <h1 className="!text-[22px] !font-semibold !text-gray-800 dark:!text-white !tracking-tight">Edit Amenity</h1>
          <p className="!text-[14px] !text-gray-500 dark:!text-gray-400 !mt-1">Update details for this amenity.</p>
        </div>
      </div>
