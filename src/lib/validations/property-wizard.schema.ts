...
export const specificationsSchema = z.object({
  ...
  propertyFacing: z.enum(['East', 'West', 'North', 'South', 'North East', 'North West', 'South East', 'South West'], { errorMap: () => ({ message: 'Please select an option' }) }),
  propertyAge: z.enum(['New', '1-3 Years', '3-5 Years', '5-10 Years', '10+ Years'], { errorMap: () => ({ message: 'Please select an option' }) }),
  furnishing: z.enum(['Furnished', 'Semi Furnished', 'Unfurnished'], { errorMap: () => ({ message: 'Please select an option' }) }).default('Unfurnished'),
  possessionStatus: z.enum(['Ready To Move', 'Under Construction', 'Immediate', 'Future Date'], { errorMap: () => ({ message: 'Please select an option' }) }),
