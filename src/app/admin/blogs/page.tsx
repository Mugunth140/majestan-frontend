...
export default function AdminBlogsPage() {
  ...
  const getStatusBadge = (status: number) => {
    if (status === 1) {
      return <span className="inline-flex! items-center! gap-1! px-2.5! py-1! rounded-full! text-xs! font-medium! bg-emerald-50! text-emerald-600!"><CheckCircle size={12} /> Published</span>;
    }
    return <span className="inline-flex! items-center! gap-1! px-2.5! py-1! rounded-full! text-xs! font-medium! bg-gray-50! text-gray-600!"><XCircle size={12} /> Draft</span>;
  };
  ...
  return (
    <div className="w-full! max-w-7xl! mx-auto! space-y-6!">
      <div className="flex! flex-col! sm:flex-row! sm:items-center! justify-between! gap-4!">
        ...
        <Link href="/admin/blogs/new" className="inline-flex! items-center! gap-2! bg-gray-900! hover:bg-gray-800! text-white! px-5! py-2.5! rounded-xl! font-medium! transition-all! shadow-sm!">
          <FileText size={18} />
          Write Post
        </Link>
      </div>
      ...
      <div className="bg-white! rounded-3xl! shadow-sm! border! border-gray-100! overflow-hidden!">
          ...
          <table className="w-full! text-sm! text-left! text-gray-500!">
            ...
            <tbody className="divide-y! divide-gray-100!">
              {loading ? (
                ...
                blogs.map((blog) => (
                  <tr key={blog.id} className="bg-white! hover:bg-gray-50/50! transition-colors! group!">
                    <td className="px-6! py-4!">
                      <div className="flex! items-center! gap-3!">
                        ...
                        <div>
                          ...
                          <div className="text-xs! text-gray-400! mt-0.5! truncate! max-w-xs!">{blog.slug}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6! py-4!">
                      <div className="text-gray-900! capitalize!">{blog.category || 'General'}</div>
                      <div className="text-xs! text-gray-400! mt-0.5!">{blog.author || 'Admin'}</div>
                    </td>
                    <td className="px-6! py-4!">
                      <div className="text-gray-900!">{new Date(blog.created_at).toLocaleDateString()}</div>
                    </td>
                    <td className="px-6! py-4!">
        ...
        {!loading && blogs.length > 0 && (
          <div className="p-5! border-t! border-gray-100! flex! items-center! justify-between!">
            <span className="text-sm! text-gray-500!">Showing <span className="font-medium! text-gray-900!">{blogs.length}</span> blogs</span>
